...
<script>
  ...
  import {sendFindPwdMail} from "../../api/common/sendFindPwdMail";
  import {findPwd} from "../../api/common/findPwd";
  ...
  export default {
    ...
    methods: {
      //发送验证码
      verificationSubmit(){
        sendFindPwdMail(this.ruleForm.email).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            findPwd(this.ruleForm.email,this.ruleForm.pass,this.ruleForm.verification).then(res=>{
              if(res.status===200){
                this.$message.success(res.data.msg);
                this.$emit('findPwdSuccess');
              }else {
