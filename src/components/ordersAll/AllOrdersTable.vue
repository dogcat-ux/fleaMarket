  ...
  <div>
    <el-table
        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="卖家名字">
              <span @click="goUser(props.row)" class="seller">{{ props.row.sellerName}}</span>
            </el-form-item>
            <el-form-item label="我对该商品的评分">
              <div>
      ...
      <el-table-column
              label="操作">
        <template slot-scope="scope">
            <span  @click="goDetailInTable(scope.$index)">
              <slot name="operation"></slot>
            </span>
          <el-button size="mini"
                     type="primary"
                     @click="toReceive(scope.$index,scope.row)"
                     v-if="scope.row.isReceived===false">确认收货</el-button>
          <span v-else>
                已收货
              </span>
        </template>
    ...
    <el-dialog
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="affirmReceive">确定收货</el-button>
        </span>
  ...
  export default {
    ...
    methods:{
      goDetailInTable(index){
        this.$emit('goDetailInTable',index)
      },
      toReceive(index,row){
        this.nowIndex = index;
        this.nowRow = row;
        ...
        // }else {
        //   upOrderIsReceived(this.rateValue,).then(res=>{
        //     console.log(res)
        //     if(res.status===200){
        //
        //     }else{
